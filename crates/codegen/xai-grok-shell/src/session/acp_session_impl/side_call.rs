...
pub(crate) struct AuxCall {
    ...
    /// Stable cache namespace for this auxiliary request, isolated from the
    /// main turn (`None` when cache routing is unavailable — best-effort).
...
impl SessionActor {
    ...
    /// needed.
    /// `cache_kind` namespaces the side-call's isolated prompt-cache route
    /// (`"recap"`, `"turn_summary"`, ...): auxiliary tokens never share or
    /// pollute the main session's cache route.
    pub(crate) async fn side_call_request(
        ...
        let prompt_cache_key = self
            .cache_routing_for_model(&setup.model)
            .await
            .map(|routing| routing.aux_prompt_cache_key(cache_kind));
