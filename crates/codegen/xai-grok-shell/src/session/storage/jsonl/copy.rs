...
impl JsonlStorageAdapter {
    ...
    pub(crate) fn copy_session_data_sync(
        ...
        let checkpoint_replacement = chat_to_copy
            .first()
            .and_then(ConversationItem::as_responses_checkpoint)
            .map(|wrapper| (format!("fork-{}", uuid::Uuid::now_v7()), wrapper.clone()));
        ...
        let mut checkpoint_files = copied_updates.checkpoint_files;
        let mut num_messages = copied_updates.count;

        // Append the fresh branch-boundary marker and complete typed-tail
        // baseline after the copied journal (or into the empty filtered one).
        if let Some((operation_id, wrapper)) = &checkpoint_replacement {
            checkpoint_files.insert(wrapper.portable_history_path.clone());
            let mut extra: Vec<SessionUpdate> = Vec::new();
            let wrap_update = |update| {
                SessionUpdate::Xai(Box::new(
                    crate::extensions::notification::SessionNotification {
                        session_id: target_info.id.clone(),
                        update,
                        meta: None,
                    },
                ))
            };
            extra.push(wrap_update(
                crate::extensions::notification::SessionUpdate::CompactionCheckpoint(Box::new(
                    crate::session::storage::responses_compaction::marker_for_wrapper(wrapper),
                )),
            ));
            ...
            let mut appender = BufWriter::new(
                std::fs::OpenOptions::new()
                    .append(true)
                    .open(self.updates_file(target_info))?,
            );
            for update in &extra {
                let envelope = SessionUpdateEnvelope::from_update(update).map_err(invalid_data)?;
                serde_json::to_writer(&mut appender, &envelope).map_err(invalid_data)?;
                appender.write_all(b"\n")?;
            }
            appender.flush()?;
            num_messages += extra.len();
        }
        drop(chat_to_copy);
