...
impl JsonlStorageAdapter {
    ...
    fn load_workflow_runs_sync(
        ...
        let mut entries: Vec<_> = std::fs::read_dir(&workflows_dir)?
            ...
            .collect();
        // Sort before capping so the kept runs are deterministic regardless
        // of the filesystem's readdir order.
        entries.sort_by_key(|entry| entry.file_name());
        entries.truncate(MAX_RESTORED_WORKFLOW_RUNS.saturating_add(1));
