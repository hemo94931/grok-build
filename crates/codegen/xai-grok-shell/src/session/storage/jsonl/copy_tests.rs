...
use crate::session::storage::{
    CopySessionOptions, JsonlStorageAdapter, SessionUpdate, SessionUpdateEnvelope, StorageAdapter,
};
...
async fn write_checkpoint_file(adapter: &JsonlStorageAdapter, info: &Info, id: &str) {
    use crate::extensions::notification::{CompactionCheckpointFile, CompactionCheckpointKind};
    adapter
...
async fn recovery_does_not_upgrade_an_unknown_checkpoint_marker() {
    ...
    let error = adapter
        .repair_responses_marker_sync(
            ...
            crate::session::storage::responses_compaction::RecoveredHistory {
...
async fn copy_session_data_copies_current_responses_sidecar() {
    ...
    .unwrap();
    let sidecar = crate::session::storage::responses_compaction::CompactionCheckpointFile::new(
        wrapper.clone(),
        material,
        portable,
        None,
    ...
    let target_history = crate::session::storage::responses_compaction::read_history(
        ...
        .filter_map(|update| {
            ...
            match &notification.update {
                crate::extensions::notification::SessionUpdate::ConversationAppendPrepared(
                    ...
                    crate::session::storage::responses_compaction::TailJournalRecord::Prepared(
                        (**prepared).clone(),
                    ),
                ),
                crate::extensions::notification::SessionUpdate::ConversationAppendCommitted(
                    ...
                    crate::session::storage::responses_compaction::TailJournalRecord::Committed(
                        committed.clone(),
                    ),
                ),
                _ => None,
    ...
    assert_eq!(
        crate::session::storage::responses_compaction::rebuild_updates_only(
            target_wrapper,
            &target_records,
        )
        .unwrap()
    ...
    let filtered_history = crate::session::storage::responses_compaction::read_history(
