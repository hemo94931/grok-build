...
impl ModelsManager {
    ...
    /// Reset a signed-out session identity while keeping usable provider models.
    ///
    ...
    fn rebuild_signed_out_catalog(&self, cfg: &config::Config) {
        ...
        if provider_models.is_empty() {
            self.rebuild_bundled(cfg);
            return;
        }

        self.inner.catalog.write().models = provider_models;
        self.reselect_current_model_if_missing(cfg);
    }
    ...
    pub(crate) async fn on_auth_changed(&self) {
        ...
        *self.inner.fetch_auth.write() = fetch_auth;
        ...
        // Prefer locally configured provider models, then upstream's bundled
        // fallback when no provider model is usable.
        if !has_session && fetch_auth == ModelFetchAuth::Session {
            self.clear();
            self.rebuild_signed_out_catalog(&config);
