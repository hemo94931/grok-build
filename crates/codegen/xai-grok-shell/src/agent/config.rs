...
pub(crate) fn sampling_config_for_model(
    ...
    // Provider routes carry their own headers (and must not get cli-chat-proxy
    // stamp headers). First-party routes keep the URL-derived inject path.
    ...
    let extra_response_includes = response_include_extensions(
        !is_provider && info.supports_backend_search,
        &api_backend,
        &credentials.base_url,
    );
    SamplerConfig {
