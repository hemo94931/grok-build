...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            // Use real temporary files instead of Unix device paths such as
            // /dev/stdout and /dev/null so this also works on Windows.
            let tempdir = tempfile::TempDir::new()
                .context("failed to create temporary directory for protoc outputs")?;
            let dependency_out = tempdir.path().join("dependencies.d");
            let descriptor_set_out = tempdir.path().join("descriptor.pb");
            let dependency_out_str = dependency_out
                .to_str()
                .context("protoc dependency output path not UTF-8")?;
            ...
            command
                .arg(format!("--dependency_out={dependency_out_str}"))
            ...
            let prefix = format!("{descriptor_set_out_str}:");
            let rem = first_line
